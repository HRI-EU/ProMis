...
class DoubtDensity:
    ...
    def fit(self, samples: NDArray, doubt_space: dict[str, dict], number_of_epochs: int, batch_size: int) -> NDArray:
        ...
        dataloader = torch.utils.data.DataLoader(torch.hstack([samples, self.doubt_space_to_tensor(doubt_space)]), batch_size=batch_size, shuffle=True)
        ...
        for epoch in tqdm(range(number_of_epochs), desc="Learning Doubt Density", unit="epoch"):
            for batch in dataloader:
                optimizer.zero_grad()
                loss = -self.flow.log_prob(batch[:, :self.number_of_states], context=batch[:, self.number_of_states:]).mean()
                loss.backward()
                optimizer.step()
