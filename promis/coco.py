...
from numpy import array, hstack, mean, newaxis
...
class DoubtDensity:
    ...
    def __init__(
        ...
        self.number_of_context_features = sum([
            1 if doubt_space[doubt_feature]['type'] == 'continuous' else doubt_space[doubt_feature]['number_of_classes']
            for doubt_feature in doubt_space.keys()
        ])

        # Need to ensure even number of features for nflows library
        if self.number_of_context_features % 2 == 1:
        ...
        self.flow = Flow(
            ...
            ConditionalDiagonalNormal(shape=[self.number_of_states])
        )
    ...
    def sample(self, size: int, doubt_space: dict[str, dict]):
        return self.flow.sample(size, context=self.context_encoder(doubt_space))

    def log_prob(self, states: NDArray, doubt_space: dict[str, dict]):
        return self.flow.log_prob(states, context=self.context_encoder(doubt_space))

    def prob(self, states: NDArray, doubt_space: dict[str, dict]):
        return self.log_prob(states, doubt_space).exp()

    def fit(self, samples: NDArray, doubt_space: dict[str, dict], number_of_epochs: int, batch_size: int) -> NDArray:
        # Setup optimizer
        optimizer = torch.optim.Adam(self.flow.parameters())
        dataloader = torch.utils.data.DataLoader(torch.hstack([samples, self.context_encoder(doubt_space)]), batch_size=batch_size, shuffle=True)
        ...
        for epoch in tqdm(range(number_of_epochs)):
            for batch in dataloader:
                optimizer.zero_grad()
                loss = -self.flow.log_prob(batch[:, 0:self.number_of_states], context=batch[:, self.number_of_states:]).mean()
                loss.backward()
                optimizer.step()
    ...
    def context_encoder(self, doubt_space: dict[str, dict]):
        ...
        for feature in doubt_space.keys():
            if doubt_space[feature]['type'] == 'categorical':
                ...
                encoded_features.append(torch.from_numpy(encoder.transform(doubt_space[feature]['values'][:].reshape(-1, 1))))
