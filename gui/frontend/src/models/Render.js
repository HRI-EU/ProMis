...
class LeafletNativeRender extends BaseRender {
  ...
  commonEnd(satFactor, currentLayer, createdMarker, marker) {
    ...
    var feature = (createdMarker.feature = createdMarker.feature || {});
...
export class HeatmapRectRender extends LeafletNativeRender {
  ...
  render(currentLayer) {
    ...
    const markers = currentLayer.markers.map((marker) => {
      ...
      var hsl = ColorHelper.calcHslFromParams(currentLayer.hue, sat, positive);
      if (
  ...
  render(currentLayer) {
    ...
    const markers = currentLayer.markers.map((marker) => {
      ...
      if (
        ...
        var createdMarker = null;
        // console.log(marker);
        var key = JSON.stringify([marker.position[0], marker.position[1]]);
        // console.log("key: ", key);
  ...
  render(currentLayer) {
    ...
    currentLayer.markers.forEach((marker, markerIndex) => {
      ...
      if (
        ...
        svgInner += `<rect x="${markerIndex % currentLayer.width}" y="${
          currentLayer.height - 1 - Math.floor(markerIndex / currentLayer.width)
        }" width="1" height="1" fill="${hsla}" />`;
      }
    });
    ...
    svgElement.setAttribute(
      "viewBox",
      `0 0 ${currentLayer.width} ${currentLayer.height}`,
    );
    ...
    const dlon =
      (currentLayer.markers[1].position[1] -
        currentLayer.markers[0].position[1]) /
      2.0;
    const dlat =
      (currentLayer.markers[currentLayer.width].position[0] -
        currentLayer.markers[0].position[0]) /
      2.0;
    ...
    let svgO = L.svgOverlay(svgElement, bbox, { interactive: false }).addTo(
      this.map,
    );
    ...
    currentLayer.markers.forEach((marker, markerIndex) => {
      ...
      if (
        ...
        pngCtx.fillRect(
          ...
          currentLayer.height -
            1 -
            Math.floor(markerIndex / currentLayer.width),
          1,
          1,
        );
      }
